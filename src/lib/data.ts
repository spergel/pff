...
/** Load ISIN → formatted description map (exported for page-level use) */
export function loadDescMap(): Map<string, string> {
  const p = path.join(DATA_ROOT, "ticker_meta.json");
  if (!fs.existsSync(p)) return new Map();
  const raw = JSON.parse(fs.readFileSync(p, "utf-8")) as Record<string, string>;
  return new Map(Object.entries(raw).map(([isin, ft]) => [isin, formatFigiDesc(ft)]));
}
