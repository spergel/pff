  ...
  return (
    <div className="space-y-3">
      <div className="sticky top-14 z-10 bg-white pb-2 flex items-center gap-3">
        <input
          type="text"
          placeholder="filter by ticker, name, sector, ISIN, CUSIP…"
          value={query}
      ...
      <div className="overflow-auto max-h-[calc(100vh-180px)] border-2 border-gray-600">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-300">
            {sorted.map((h, i) => {
              const lookupKey = h.cusip || h.ticker;
              const lookupHref = `/security?q=${encodeURIComponent(lookupKey)}`;
              return (
                <tr
                  key={h.isin}
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    ...
                    {h.desc && <div className="truncate max-w-[200px] text-[10px] text-gray-400">{h.desc}</div>}
                  </td>
